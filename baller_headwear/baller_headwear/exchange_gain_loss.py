                ...
                if args:
                    ...
                    for arg in args:
                        if (
                            ...
                            # Fix: Select correct Gain/Loss accounts for Suppliers
                            if difference_amount > 0:
                                gain_loss_account = (
                                    frappe.get_cached_value("Company", self.company, "custom_exchange_gain_account")
                                    if arg.get("party_type") == "Customer"
                                    else frappe.get_cached_value("Company", self.company, "custom_exchange_loss_account")
                                )
                            else:
                                gain_loss_account = (
                                    frappe.get_cached_value("Company", self.company, "custom_exchange_loss_account")
                                    if arg.get("party_type") == "Customer"
                                    else frappe.get_cached_value("Company", self.company, "custom_exchange_gain_account")
                                )

                            # Keep debit/credit logic unchanged
                            dr_or_cr = "debit" if difference_amount > 0 else "credit"
                            ...
                            if not self.gain_loss_journal_already_booked(
                                ...
                                frappe.msgprint(
                                    _(f"Exchange Gain/Loss amount has been booked through {get_link_to_form('Journal Entry', je)}")
                                )
